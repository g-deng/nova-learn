...
async def generate_topics(
    ...
    if not stack_info:
        raise HTTPException(
            status_code=404, detail="Stack not found or does not belong to user"
        )
...
async def submit_topic_list(
    stack_id: uuid.UUID,
    body: SubmitTopicListRequest,
...
@router.post(
    "/{stack_id}/submit_dependencies",
    response_model=dict[str, tuple[uuid.UUID, uuid.UUID]],
)
async def submit_dependencies(
    body: SubmitDependenciesRequest,
    user=Depends(get_current_user),
    db: Session = Depends(get_db),
):
    new_ids = {}
    for dep in body.new_dependencies:
        try:
            ...
            if not added_dep:
                raise HTTPException(
                    status_code=404,
                    detail="One or more topics not found or do not belong to user",
                )
            new_ids[dep[0] + "," + dep[1]] = (
                added_dep.from_topic_id,
                added_dep.to_topic_id,
            )
    ...
    for ids, (new_from, new_to) in body.old_dependencies.items():
        ...
        try:
            if not crud.update_topic_dependency(
                db, from_id, to_id, new_from, new_to, user.id
            ):
                raise HTTPException(
                    status_code=404,
                    detail="One or more topics not found or do not belong to user",
    ...
    for ids in body.deleted_dependencies:
        ...
        try:
            if not crud.delete_topic_dependency(db, from_id, to_id, user.id):
                raise HTTPException(
                    status_code=404,
                    detail="Dependency not found or does not belong to user",
                )
        except Exception as e:
