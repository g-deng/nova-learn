      ...
      try {
        ...
        const parsedDependencies = dependenciesResult.data.map((d: any) => ({
          from:
            ...
            "error",
          to:
            parsedTopics.find((t: any) => t.id == d.to_topic_id)?.name ||
            "error",
    ...
    <div className="h-full w-full flex border border-red-500">
          ...
          <Button
            ...
            disabled={
              (dependencies.length > 0 &&
                (dependencies[dependencies.length - 1].from === "" ||
                  dependencies[dependencies.length - 1].to === "")) ||
              undefined
            }
          >
            New Dependency
      ...
      <div className="flex-1 h-full pb-4 flex flex-col border border-red-500">
        ...
        <p className="text-sm text-gray-500 pb-2">
          Only shows saved dependencies.
        </p>
        <GraphViewer
          nodes={topics}
          links={dependencies
            .filter((dep) => dep.fromId && dep.toId)
            .map(
              (dep) =>
                ({
                  source: dep.fromId,
    ...
    >
      <div className={"flex items-center justify-between p-2 gap-6 border-b "}>
        <Select onValueChange={(newVal) => editFromTopic(index, newVal)}>
          <SelectTrigger
            className={"w-[180px] " + (dependency.fromId ? "bg-gray-200" : "")}
          >
        ...
        <Button onClick={() => deleteDependency(index)} variant="destructive">
          Delete
        </Button>
      </div>
      <div className="pt-2">
        {dependency.fromId && touched && (
          <Alert variant="destructive">
            <AlertTitle>You are editing an existing dependency</AlertTitle>
          </Alert>
        )}
      </div>
    </div>
