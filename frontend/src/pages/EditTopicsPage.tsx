      ...
      try {
        const saveResult = await axios.post(
          `${import.meta.env.VITE_BACKEND_URL}/stacks/${stackId}/submit_topic_list`,
          {
            ...
            deleted_topics: saved
              .filter((t) => t.id && !topics.some((topic) => topic.id === t.id))
              .map((t) => t.id)
          },
          {
            headers: {
  ...
  return (
    <div className="w-full h-full overflow-auto">
      ...
      <div className="flex items-center gap-4 pb-4">
        <Button onClick={generateTopics} disabled={loading || unsavedChanges}>
          Generate Topics
        </Button>
        <Button onClick={submitTopics} disabled={loading || !unsavedChanges}>
          Save Changes
        </Button>
        <Button
          onClick={() => guardedNavigate(`/stack/${stackId}/edit-dependencies`)}
          disabled={loading}
        >
          Edit Dependencies
        </Button>
      ...
      <div className="flex flex-col gap-2 pt-2">
        {topic.id && touched > 0 && (
          <Alert variant="destructive">
            <AlertTitle>You are editing an existing topic</AlertTitle>
          </Alert>
        )}
